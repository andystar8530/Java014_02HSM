	...
	<table style="margin-left:auto; margin-right:auto; width:810; background:#F5EBFF; border:2px solid blue; border-style: outset; ">
		...
		<c:forEach var="anOrderBean" varStatus="stat" items="${memberOrders}">
			...
			<TD id='borderA' width="86" align="center">
			    <a  href='<c:url value='orderDetail?m_No=${LoginOK.m_No}&orderNo=${anOrderBean.s_OrderNo}' />'>
				    ${anOrderBean.s_OrderNo}
			    </a>
			</TD>
<%-- 			<TD id='borderA' width="100" align="center">${anOrderBean.s_OrderDate}</TD> --%>
			<TD id='borderA' width="100" align="center"><fmt:formatDate value='${anOrderBean.s_OrderDate}' type='date'/></TD>
<%-- 			<TD id='borderA' width="80" align="right">${anOrderBean.s_OrderSum}</TD> --%>
			<TD id='borderA' width="80" align="right"><fmt:formatNumber value="${anOrderBean.s_OrderSum}" pattern="#,###,###" />元</TD>
			<TD id='borderA' width="380" align="left">&nbsp;${anOrderBean.s_M_Address}</TD>
			<TD id='borderA' width="80" align="left">&nbsp;
			<c:if test="${anOrderBean.s_Status ==0}">收到訂單</c:if>
			<c:if test="${anOrderBean.s_Status ==1}">撿貨</c:if>
			<c:if test="${anOrderBean.s_Status ==2}">理貨</c:if>
			<c:if test="${anOrderBean.s_Status ==3}">出貨</c:if>
			<c:if test="${anOrderBean.s_Status ==4}">已送達</c:if>
			</TD>
