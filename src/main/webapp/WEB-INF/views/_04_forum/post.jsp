			...
			<div class="media text-muted pt-3  ">
				<div class="d-block">
					<img
						...
						data-holder-rendered="true"> <strong
						class="d-block text-gray-dark p-2  ">${post.memberBean.newlywedBean.n_nickname}</strong>
				</div>
				
				
				<div class="container-fluid">
					<p class="media-body pb-3 mb-0  lh-125 border-bottom border-gray"
						" style="display: block;">${post.fTitle}</p>

					<p class="media-body pb-3 mb-0  lh-125 border-bottom border-gray ">${post.fText}</p>

					<%-- 					<%=path%>/_04_forum/like?postId=${post.fId}&tf=true">${post.fLike} --%>
					<%-- 					<%=path%>/_04_forum/like?postId=${post.fId}&tf=false">${post.fHate} --%>
					...
					<c:if test="${loh.likeOrHate==0 || empty loh.likeOrHate}">
						<div class="col-12 article-mood">
							<ul class="like-or-hate">
								...
								<li id="like1" button class="fas fa-thumbs-up fa-2x"><a
									href="<%=path%>/_04_forum/like?postId=${post.fId}&loh=${loh.no}&tf=1">${post.fLike}</a></li>

								<li id="hate1"  class="fas fa-thumbs-down fa-2x"><a
									href="<%=path%>/_04_forum/like?postId=${post.fId}&loh=${loh.no}&tf=2">${post.fHate}</a></li>
							</ul>
						</div>
					</c:if>
						...
						<div class="col-12 article-mood">
							<ul class="like-or-hate">
								...
								<li id="like1" button class="fas fa-thumbs-up fa-2x"
									type="button" style="color: red"><a
									href="<%=path%>/_04_forum/like?postId=${post.fId}&loh=${loh.no}&tf=0">${post.fLike}</a></li>
					...
					<c:if test="${loh.likeOrHate==2}">
						<div class="col-12 article-mood">
							<ul class="like-or-hate">
								...
								<li id="hate1" button class="fas fa-thumbs-down fa-2x"
									type="button" style="color: red"><a
									href="<%=path%>/_04_forum/like?postId=${post.fId}&loh=${loh.no}&tf=0">${post.fHate}</a></li>
							</ul>
						</div>
					</c:if>
			...
			<c:forEach var="comment" items="${getComments}">
				<div class="media text-muted pt-3">
					<img alt="32x32" class="mr-2 rounded"
						src="${pageContext.request.contextPath}/_00_init/getMemberImage?id=${comment.memberBean.m_Id}"
						data-holder-rendered="true" style="width: 32px; height: 32px;">
					<div
						class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
						<div
							class="d-flex justify-content-between align-items-center w-100">
							<strong class="text-gray-dark">${comment.memberBean.newlywedBean.n_nickname}</strong>

							<form id="form1" name="form1" method="post" action="">
								<input type="image" name="submit_Btn" id="submit_Btn" img
									src="https://www.crazy-tutorial.com/wp-content/uploads/2018/08/6052.png"
			...
			<form:form method="Post" modelAttribute="formCb">
				...
				<small class="d-block text-right mt-3">
					<div class="input-group mb-5 mt-5 col-10">
						<form:textarea path="comText" class="form-control"
							placeholder="說說你的看法" aria-describedby="button-addon2" />
						<div class="input-group-append">
							<button class="btn btn-outline-danger" type="submit"
								id="button-addon2">留言</button>
						</div>
					</div>
				</small>
			</form:form>
