...
import partner_h._01_profolios.validator.PfValidator;
...
@Controller
@SessionAttributes({"partnerBean","pfmId","p_id","portfolioBean"})
public class PFManagementAjaxController {
	...
	public String showPortfolioInfo(Model model,
		...
		model.addAttribute("portfolioBean", pfBean);
...
//			System.out.println("============="+pfmId);
	...
	//改用SpringMVC修改資料
	@PostMapping("/editPortfolioInfo")
	public String portfolioInfoEdit(
			@ModelAttribute("portfolioBean") PortfoliosBean portfolioBean ,			
				BindingResult result) {
	PfValidator validator = new PfValidator();
	validator.validate(portfolioBean, result);
	if(result.hasErrors()) {
		return "/portfolioEdit/"+portfolioBean.getPfmId();		
	}
	
	portfoliosService.updatePortfolio(portfolioBean);
//	return "redirect:/portfolioEdit/"+portfolioBean.getPfmId();
	return "redirect:/showPortfoliosManagement";
