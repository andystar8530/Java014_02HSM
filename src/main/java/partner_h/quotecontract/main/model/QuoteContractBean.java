...
public class QuoteContractBean {
	...
	@OneToMany(mappedBy="quoteContractBean",cascade=CascadeType.ALL)
	Set<QutoeContractItemBean>items = new LinkedHashSet<>(); //訂單明細

	public QuoteContractBean() {
		super();
	}

	public QuoteContractBean(Integer qcId, String qcName, Date qcDate, Date qcDateLine, String qcStatus, Clob qcContent,
			Integer n_Id, Blob n_Signature, Integer p_Id, Blob p_Signature, Date qcExecutionDate, Integer qcTotalAmount,
			Double qcDepositRate, Integer qcDeposit, Clob qcNotes, Set<QutoeContractItemBean> items) {
		this.qcId = qcId;
		this.qcName = qcName;
		this.qcDate = qcDate;
	...
	public Clob getQcContent() {
		return qcContent;
	}

	public void setQcContent(Clob qcContent) {
		this.qcContent = qcContent;
	}
	...
	public Clob getQcNotes() {
		return qcNotes;
	}

	public void setQcNotes(Clob qcNotes) {
		this.qcNotes = qcNotes;
	}

	public Set<QutoeContractItemBean> getItems() {
		return items;
	}

	public void setItems(Set<QutoeContractItemBean> items) {
		this.items = items;
	}
